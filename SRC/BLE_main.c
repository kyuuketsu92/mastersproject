...
static received_command_callback nus_callback = NULL;
...
static void nus_data_handler(ble_nus_evt_t * p_evt)
{
    ...
    if (p_evt->type == BLE_NUS_EVT_RX_DATA)
    {
        ...
        if(nus_callback != NULL)
        {
            nus_callback(m_nus_data_array, BLE_NUS_MAX_DATA_LEN, 1);
        }
    }
}
